//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import <AbstractSketchPlugin/GPBMessage.h>

@class NSMutableArray, PRORPCTraceTimestamp;

@interface PRORPCTraceLog : GPBMessage
{
}

+ (id)descriptor;

// Remaining properties
@property(retain, nonatomic) NSMutableArray *fieldsArray; // @dynamic fieldsArray;
@property(readonly, nonatomic) unsigned long long fieldsArray_Count; // @dynamic fieldsArray_Count;
@property(nonatomic) BOOL hasTimestamp; // @dynamic hasTimestamp;
@property(retain, nonatomic) PRORPCTraceTimestamp *timestamp; // @dynamic timestamp;

@end

